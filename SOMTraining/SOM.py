...
class SOM(object):
    ...
    def __init__(self, m, n, dim, n_iterations=100, alpha=None, sigma=None):
        ...
        self._n = n
    		...
    		# Eğer alpha boş ise 0.3 ver değilse aldığın rakamı float yap ve alpha ya ata
        if alpha is None:
            alpha = 0.3
        else:
            alpha = float(alpha)
    		...
    		# Eğer sigma boş ise SOM ağının en veya boyundan büyük olanının yarısını sigma olarak belirle. else verilen sigma değerini float yap.
            if sigma is None:
                sigma = max(m, n) / 2.0
            else:
                sigma = float(sigma)
        ...
        with self._graph.as_default():
            ...
            bmu_index = tf.argmin(tf.sqrt(tf.reduce_sum(
                tf.pow(tf.subtract(self._weightage_vects, tf.sack([self._vect_input for i in range(m*n)])), 2), 1)),0)
